...
// ---------------------------------------------------------------------------
// Note on the HDF5 WebAssembly runtime
// ---------------------------------------------------------------------------
// This plugin reads netCDF4/HDF5 (EMIT) scenes with `h5wasm`. Its ESM build is a
// single-file build that embeds the `.wasm` binary directly in the JS, so it is
// inlined into `dist/index.js` automatically and there is NO separate `.wasm`
// asset to ship or resolve at runtime. The recipe below is therefore unused, but
// kept for any future static assets (sample data, colormaps, etc.).
//
// ---------------------------------------------------------------------------
