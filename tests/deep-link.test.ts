...
describe('getPluginDataValue', () => {
  it('returns the value when the parameter is present', () => {
    expect(
      getPluginDataValue(params('?hypercoast-data=https://example.com/dataset.zip')),
    ).toBe('https://example.com/dataset.zip');
  });

  it('percent-decodes the value', () => {
    expect(getPluginDataValue(params('hypercoast-data=https%3A%2F%2Fx%2Fy.zip'))).toBe(
      'https://x/y.zip',
    );
  });

  it('ignores other parameters', () => {
    expect(getPluginDataValue(params('a=1&hypercoast-data=https://x/y.zip&b=2'))).toBe(
      'https://x/y.zip',
    );
  });
  ...
  it('returns null when the parameter is blank or whitespace', () => {
    expect(getPluginDataValue(params('?hypercoast-data='))).toBeNull();
    expect(getPluginDataValue(params('?hypercoast-data=%20%20'))).toBeNull();
  });

  it('exposes the parameter name', () => {
    expect(PLUGIN_DATA_PARAM).toBe('hypercoast-data');
  });
});
...
describe('maybeHandleDeepLink', () => {
  it('forwards the value when the parameter is present', async () => {
    ...
    await maybeHandleDeepLink(consumer, params('?hypercoast-data=https://x/y.zip'));
  ...
  it('does nothing when the parameter is absent or blank', async () => {
    ...
    await maybeHandleDeepLink(consumer, params('?hypercoast-data='));
